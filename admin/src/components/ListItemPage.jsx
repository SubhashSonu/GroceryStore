...
const ListItemPage = () => {
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        const response = await axios.get("https://grocerystore-backend-57i5.onrender.com/api/items");
        const data = response.data;

        const withUrls = data.map((item) => ({
          ...item,
          imageUrl: item.imageUrl
            ? `https://grocerystore-backend-57i5.onrender.com${item.imageUrl}`
            : null,
        }));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://grocerystore-backend-57i5.onrender.com/api/items/${id}`);
